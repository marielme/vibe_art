...
function drawBrushArt() {
    ...
    for (let i = 0; i < pose.keypoints.length; i++) {
        ...
        if (keypoint.confidence > 0.4) {
            ...
            if (prevKeypointPositions[i]) {
                ...
                if (speed > 15) {
                    ...
                    for (let j = 0; j < numSplatters; j++) {
                        ...
                        let hue = map(i, 0, pose.keypoints.length, 200, 340);
            ...
            if (trailPoints[i].length > 2) {
                // Color based on keypoint type and movement
                let hue = map(i, 0, pose.keypoints.length, 200, 340);
                let sat = 60;
                let bri = 80;
                let alpha = 150;
    ...
    if (frameCount % 2 === 0) {
        for (let connection of connections) {
            ...
            if (a && b && a.confidence > 0.3 && b.confidence > 0.3) {
                ...
                // Soft spray effect for skeleton
    ...
    for (let keypoint of pose.keypoints) {
        if (keypoint.confidence > 0.5 && frameCount % 4 === 0) {
            ...
            // Wrists get special bright effect
            if (keypoint.name.includes('wrist')) {
                noStroke();
                fill(random(280, 320), 80, 95, 180);
                circle(x, y, random(15, 35));
                fill(random(180, 220), 70, 90, 120);
